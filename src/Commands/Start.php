    ...
    public function execute()
    {
        ...
        foreach ($tarsServerConfig['adapters'] as $key => $adapter) {
            ...
            if (!isset($serviceInfo['serverType'])) {
                $servicesInfo[$adapter['objName']]['serverType'] = 'not_tars' == $adapter['protocol'] || 'not_taf' == $adapter['protocol'] ? 'http' : 'tcp';
            }

            if (!isset($serviceInfo['protocolName'])) {
                $servicesInfo[$adapter['objName']]['serverType'] = 'not_tars' == $adapter['protocol'] || 'not_taf' == $adapter['protocol'] ? 'http' : 'tars';
            }
...
//        $server->getSw()->is_my_tars=true;

//        $server->ws->is_my_tars=true;
    }
}
